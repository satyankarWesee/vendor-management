...
export default function EmployeeRegistration() {
  ...
  // Fetch unique departments on component mount
  useEffect(() => {
  ...
  // Fetch specializations when department changes
  useEffect(() => {
    const fetchSpecializations = async () => {
      if (employee.department) {
        try {
          const response = await axios.get(`${apiUrl}/admin/employees/specializations/${employee.department}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
        ...
        setSpecializations([]); // Reset specializations if no department is selected
      }
    };

    fetchSpecializations();
  }, [employee.department, apiUrl, token]);
        ...
        <form onSubmit={handleSingleSubmit} className="space-y-4">
          ...
          <div className="file-upload-section">
            ...
            <input
              ...
              accept=".xlsx, .xls"
              className="mt-4" // This adds margin top to the input field
            />
            <div className="mt-4"> {/* Add a div to create margin above the button */}
              <Button type="button" onClick={handleUpload} className="bg-green-500 text-white">
                Upload File
              </Button>
            </div>
          </div>
